...
const runAll = (tasks, context) => {
  for (const task of tasks) {
    ...
    try {
      ...
      if (task.showError) atom.notifications.addError(`[${task.title}]`, { detail: 'Something went wrong.', dismissable: true })
    }
  }
}
...
const runCmd = async (title, command, cursor, context) => {
  const { cmd, showOutput, showError, preferLocal, appendFile, timeout } = command
  ...
  if (result.failed || result.killed || result.signal != null) {
    debug('Failure', cmd, Object.assign({}, result, { message: result.message }))
    if (showError) {
      let detail = result.stderr || result.message
      if (result.timedOut) detail = `Timed out: it took more than ${(timeout / 1000).toFixed(2)}s\n \n` + detail
      detail = `CWD: ${rootPath}\n \n` + detail
