...
const isSimple = task => {
  const res = isString(task) || (isArray(task) && isArrayOfString(task))
  debug('isSimple', task, res)
  return res
}

const isClassic = (task, allowedOptions) => {
  const res = isObject(task) && isValidObject(task, allowedOptions) && isSimple(task.cmd)
  debug('isClassic', task, allowedOptions, res)
  return res
}

const isExtended = (task, allowedOptions) => {
  const res = Array(task) && isArrayOfObject(task) && task.every(t => isClassic(t, allowedOptions))
  debug('isExtended', task, allowedOptions, res)
  return res
}

const simpleToExpended = (commands, options) => {
  const res = castArray(commands).reduce(
    (acc, cmd) => (isEmpty(cmd) ? acc : acc.concat([{ cmd, ...options }])),
    []
  )
  debug('simpleToExpended', commands, options, res)
  return res
}

const classicToExpended = (commands, options) => {
  const { cmd, ...opts } = commands
  const res = simpleToExpended(cmd, { ...options, ...opts })
  debug('classicToExpended', commands, options, res)
  return res
}
...
const normalizeExpended = (commands, options) => {
  ...
  debug('normalizeExpended', commands, options, normalized)
  return normalized
}

const includes = (options, value) => {
  if (!isArray(options) || !isString(value)) throw new Error('invalid signature')
  const res = options.includes(value)
  debug('includes', options, value, res)
  return res
}

const isArrayOfString = value => {
  if (!isArray(value)) throw new Error('invalid signature')
  const res = value.every(e => isString(e))
  debug('isArrayOfString', value, res)
  return res
}

const isArrayOfObject = value => {
  if (!isArray(value)) throw new Error('invalid signature')
  const res = value.every(e => isObject(e))
  debug('isArrayOfObject', value, res)
  return res
}

const isValidObject = (value, allowedOptions) => {
  if (!isArray(allowedOptions) || !isObject(value)) throw new Error('invalid signature')
  const res = Object.keys(value).every(key => includes(allowedOptions, key))
  debug('isValidObject', value, allowedOptions, res)
  return res
}
...
const normalizeConfig = (config, options = defaultOptions) => {
  ...
  debug('Normalizing config', config, normalized)
  return normalized
}
