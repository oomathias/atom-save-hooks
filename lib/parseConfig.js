...
const isExtended = (task, allowedOptions) => {
  ...
  return Array(task) &&
    isArrayOfObject(task) &&
    task.every((t) => isClassic(t, allowedOptions))
}

const simpleToExpended = (commands, options) => {
  debug('simpleToExpended', commands, options)
  return castArray(commands)
    .reduce((acc, cmd) => isEmpty(cmd) ? acc : acc.concat([{ cmd, ...options }]), [])
}
...
const isValidObject = (value, allowedOptions) => {
  ...
  return Object
    .keys(value)
    .every((key) => includes(allowedOptions, key))
}
...
const isValidConfig = (config) => {
  ...
  const isValidSimple = (glob, cmd) => {
    if (isArray(cmd)) {
      cmd.forEach(c => {
        if (!isString(c)) {
          errors.push(`${JSON.stringify(glob)}: Each cmd should be a string. ${JSON.stringify(c)} isn't`)
        }
      })
    } else if (!isString(cmd)) {
      errors.push(`${JSON.stringify(glob)}: cmd should be a string or array. ${JSON.stringify(cmd)} isn't`)
    }
  }
  const isValidClassic = (glob, task) => {
    ...
    if (!isDefined(task.cmd)) {
      errors.push(`${JSON.stringify(glob)}: 'cmd' is required if you use a classic or extended format`)
    } else {
      isValidSimple(glob, task.cmd)
    }
    if (isDefined(task.showOutput) && !isBoolean(task.showOutput)) errors.push(`${JSON.stringify(glob)}: showOutput should be a boolean`)
    if (isDefined(task.showError) && !isBoolean(task.showError)) errors.push(`${JSON.stringify(glob)}: showError should be a boolean`)
    if (isDefined(task.preferLocal) && !isBoolean(task.preferLocal)) errors.push(`${JSON.stringify(glob)}: preferLocal should be a boolean`)
    if (isDefined(task.appendFile) && !isBoolean(task.appendFile)) errors.push(`${JSON.stringify(glob)}: appendFile should be a boolean`)
    if (isDefined(task.timeout) && !isNumber(task.timeout)) errors.push(`${JSON.stringify(glob)}: timeout should be a number in milliseconds`)
  }
  ...
  if (!isEmpty(errors)) {
    errors.push(` \nYou can get more information RTFM!`)
